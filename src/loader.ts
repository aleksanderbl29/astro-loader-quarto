...
import { parseQuartoYaml, extractListings, getOutputDir, findListing, validateQuartoFormat } from './parsers/quarto-yaml.js';
import { resolveListing, applySortConfiguration } from './parsers/listing-config.js';
...
import { readRenderedMarkdown, matchQmdToMd } from './parsers/markdown-content.js';
...
import { renderQuarto, normalizeAutoRenderConfig } from './utils/quarto-renderer.js';
...
export function quartoLoader(config: QuartoLoaderConfig): Loader {
  ...
  const qmdCache = new FileCache<ReturnType<typeof extractMetadata>>(cacheEnabled ? 500 : 0);
  ...
  return {
    ...
    load: async ({ store, logger: astroLogger, meta }) => {
      ...
      try {
        ...
        logger.info(`Loading ${listingsToLoad.length} listing(s): ${listingsToLoad.map(l => l.id).join(', ')}`);
        ...
        for (const listing of listingsToLoad) {
          ...
          logger.debug(`Found ${resolved.files.length} file(s) for listing ${listing.id}`);
          ...
          const parsedFiles = parallelEnabled
            ...
            : await resolved.files.reduce(async (acc, file) => {
                const results = await acc;
                results.push(await parseFile(file));
                return results;
              }, Promise.resolve([] as Array<{ filePath: string; metadata: ReturnType<typeof extractMetadata> }>));

          logger.debug(`Parsed ${parsedFiles.length} file(s) for listing ${listing.id}`);

          // 7. Normalize metadata and apply field mappings
          // Track file paths for later markdown content reading
          const filePathMap = new Map<string, string>();

          const normalizedEntries = parsedFiles.map(({ filePath, metadata }) => {
          ...
          logger.debug(`Normalized ${normalizedEntries.length} entry(s) for listing ${listing.id}`);
          ...
          if (config.filter) {
            ...
            logger.debug(`Filtered to ${filteredEntries.length} entry(s) for listing ${listing.id}`);
          }
          ...
          if (config.transform) {
            ...
            logger.debug(`Transformed ${transformedEntries.length} entry(s) for listing ${listing.id}`);
          }

          // 10. Apply sort configuration
          const sortedEntries = applySortConfiguration(transformedEntries, listing.sort);

          // 11. Generate schema
          const entryDataList = sortedEntries.map(e => e.data as Record<string, unknown>);
          const schema = createListingSchema(listing, entryDataList, config.schema, logger);
            ...
            try {
              ...
              if (filePath) {
                ...
                try {
                  ...
                  logger.warn(`Could not read markdown content for ${entry.id}: ${(error as Error).message}`);
              ...
              logger.error(`Failed to process ${entry.id}: ${(error as Error).message}`);
        ...
        if ((meta as any).mode === 'dev') {
          if (!fileWatcher.isWatching()) {
            ...
            fileWatcher.on('change', (event) => {
              if (event.path.endsWith('.qmd') || event.path.endsWith('_quarto.yml')) {
        ...
        logger.error(`Failed to load Quarto content: ${(error as Error).message}`);
        throw error;
      }
    },
