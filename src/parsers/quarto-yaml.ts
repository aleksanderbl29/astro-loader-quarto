  ...
  try {
    ...
    if (!config || typeof config !== "object") {
      throw new QuartoConfigError(
        "Invalid Quarto configuration: expected YAML object",
        path,
      );
    }
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      throw new QuartoConfigError(
        `Quarto configuration file not found: ${path}`,
        path,
      );
    }
...
export function extractListings(config: QuartoConfig): QuartoListing[] {
  ...
  const listings = Array.isArray(config.listing)
    ? config.listing
    : [config.listing];
